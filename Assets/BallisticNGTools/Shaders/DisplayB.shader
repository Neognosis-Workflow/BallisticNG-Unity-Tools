Shader "BallisticNG/Display (Double Sided)"
{
    Properties
    {
        _MainTex("Base", 2D) = "white" {}
		_ScanLines("Scan Lines", 2D) = "white" {}
		_Illum("Illumination", 2D) = "black" {}
		_IllumIntensity("Illumination Intensity", Range(0, 1)) = 1
		_Color("Color", Color) = (0.5, 0.5, 0.5, 1)
		_FlickerSpeed("Flicker Speed", Range(0, 10000)) = 1
		_FlickerStrength("Flicker Strength", Range(0, 1)) = 0.4
		_ScanSpeed("Scanline Speed", Range(0, 10)) = 1

		/*---Affine Settings--*/
		[Toggle(_ALLOW_AFFINE_MAPPING)] _AllowAfineMapping("Allow Affine Mapping", Float) = 1
		_AffineBlend("Affine Blend", Range(0, 1)) = 1
    }
		Category{
			Cull Off
			SubShader
			{
				Pass
				{
					CGPROGRAM
					#include "UnityCG.cginc"
					#include "BallisticNG.cginc"

					#pragma multi_compile __ _ALLOW_AFFINE_MAPPING
					#pragma vertex vert
					#pragma fragment frag

					struct v2f
					{
						float4 vertex : SV_POSITION;
						half2 uv : TEXCOORD0;
						half distance : TEXCOORD1;

						#if defined(_ALLOW_AFFINE_MAPPING)
							half4 affineuv : TEXCOORD2;
						#endif

						fixed4 color : COLOR;
					};

					sampler2D _MainTex;
					sampler2D _ScanLines;
					float4 _MainTex_ST;
					sampler2D _Illum;
					float _IllumIntensity;
					float4 _Color;
					fixed2 scanlineOffset;
					fixed2 scanlineDistortion;
					float _FlickerSpeed;
					float _FlickerStrength;
					float _ScanSpeed;
					fixed _Cutoff;

					float flickerAmount;

					/*---Retro Settings---*/
					half _RetroClippingDistance;
					half _AffineTextureMapping;
					half _AffineBlend;
					half _VertexSnapping;

					v2f vert(appdata_full v)
					{
						v2f o;

						/*---Vertex Position---*/
						float4 vert = mul(UNITY_MATRIX_MV, v.vertex);

						float4 ps1Vert = TruncateVertex(v.vertex, GEO_RES);
						vert = lerp(vert, ps1Vert, _VertexSnapping);

						float4 outVert = mul(UNITY_MATRIX_P, vert);
						o.vertex = outVert;

						/*---Distance To Camera---*/
						o.distance = GetClippingDistance(v.vertex, 0.3, _VertexSnapping);

						/*---Color---*/
						o.color = v.color * _Color;

						/*---Uvs---*/
						half2 uv = TRANSFORM_TEX(v.texcoord, _MainTex);

						#if defined(_ALLOW_AFFINE_MAPPING)
							o.uv = uv;
							o.affineuv = CalculateAffineUvs(_AffineTextureMapping, uv, o.vertex);
						#else
							o.uv = uv;
						#endif

						return o;
					}

					fixed4 frag(v2f i) : SV_Target
					{
						/*--Uv---*/
						float2 uv;

						#if defined(_ALLOW_AFFINE_MAPPING)
							uv = lerp(i.uv, i.affineuv.xy / i.affineuv.z, i.affineuv.w * _AffineBlend);
						#else
							uv = i.uv.xy;
						#endif

						flickerAmount = sin(_Time * (_FlickerSpeed * 100));
						scanlineOffset.y = _Time * -_ScanSpeed;
						scanlineOffset.x = sin(_Time * _ScanSpeed) * 10;
						fixed4 scanLines = tex2D(_ScanLines, (uv * 7) + scanlineOffset);
						scanlineDistortion.x = 0.001f * scanLines.rgb;
						scanlineDistortion.y = 0.001f * scanLines.rgb;

						fixed4 col = (tex2D(_MainTex, uv + scanlineDistortion) * i.color);
						float alphaTemp = col.a;
						col += tex2D(_Illum, uv + scanlineDistortion) * _IllumIntensity;
						col.a = alphaTemp;

						col.rgb *= 1.0f - (scanLines.rgb * 0.2f);
						col.rgb *= (1.0f - (flickerAmount * _FlickerStrength));

						clip(i.distance < _RetroClippingDistance ? 1 : -1);
						return col;
					}

					ENDCG
				}
			}
		}
}
