Shader "BallisticNG/Building (Reflection Probe)"
{
    Properties
    {
			...
			SubShader
			{
				Pass
				{
					...
					fixed4 frag(v2f i) : SV_Target
					{
						...
						return (tex2D(_MainTex, uv) * i.color + float4(DecodeHDR(cube, unity_SpecCube0_HDR).rgb, 1)) + (tex2D(_Illum, uv) * _IllumIntensity);
					}
