...
namespace Vlm
{
    public class VlmWindow : EditorWindow
    {
        ...
        [MenuItem("BallisticNG/Vertex Lightmapper")]
        ...
        private void UnityLightingCheck()
        {
            ...
            {
                bool disable = EditorUtility.DisplayDialog("Notice", "Unity's lightmapper is currently enabled. Would you like to disable it? (strongly recommended)", "Yes", "No");
                if (disable)
                {
        ...
        {
            if (VlmBakeData.LegacyBake) EditorGUILayout.HelpBox("Settings not available in legacy bake mode.", MessageType.Warning);
            GUI.enabled = !VlmBakeData.LegacyBake;
            ...
            GUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                GUI.enabled = VlmBakeData.BounceLight && !VlmBakeData.LegacyBake;
                ++EditorGUI.indentLevel;
        ...
        private void DrawBakeButtons()
        {
            ...
            GUILayout.BeginVertical(EditorStyles.helpBox);
            {
                VlmBakeData.LegacyBake = EditorGUILayout.Toggle("Use Legacy Lightmapper", VlmBakeData.LegacyBake);

                GUILayout.Space(10);
                if (GUILayout.Button("Bake Lighting", EditorStyles.toolbarButton))
                {
                    if (VlmBakeData.LegacyBake)
                    {
                        SaveSettings();
                        BallisticUnityTools.Lightmapping.Vlm.StartBake();
                    }
                    else
                    {
                        SaveSettings();
        ...
        private void DrawTools()
        {
            ...
            GUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                if (GUILayout.Button("Upgrade components from legacy lightmapper", EditorStyles.toolbarButton)) UpgradeScene();
                --EditorGUI.indentLevel;
            }
            GUILayout.EndVertical();
        ...
        {
            bool doIt = EditorUtility.DisplayDialog("Continue?", "This will upgrade all components in your scenes to support the new lightmapper and isn't undoable. Please note that you will still need to manually expose objects to VLM yourself.", "Yes", "No");
            if (!doIt) return;

            foreach (LightSponge oldSponge in FindObjectsOfType<LightSponge>())
            {
                AdvancedLightSponge newSponge = oldSponge.gameObject.AddComponent<AdvancedLightSponge>();
                newSponge.ConvertFromOldSponge(oldSponge);

                DestroyImmediate(oldSponge);
            }

            foreach (LightmappingOptions oldOptions in FindObjectsOfType<LightmappingOptions>())
            {
                VlmBakeOptionsComponent newOptions = oldOptions.gameObject.AddComponent<VlmBakeOptionsComponent>();
                newOptions.BakeOptions = new VlmBakeOptions();
                newOptions.ConvertFromOldOptions(oldOptions);

                DestroyImmediate(oldOptions);
            }
        }
