    ...
    public struct VlmBakeTask
    {
        ...
        public static void Bake()
        {
            ...
            try
            {
                ...
                List<VlmData> vcmData = new List<VlmData>();
                ...
                int count = BakeData.SceneryMeshes.Count;
                for (int i = 0; i < count; ++i)
                {
                    /*---Progress Report---*/
                    VlmMeshObject meshObj = BakeData.SceneryMeshes[i];
                ...
                /*---Bake TRM Lighting---*/
                count = BakeData.TrmMeshes.Count;
                for (int i = 0; i < count; ++i)
                {
                    VlmMeshObject meshObj = BakeData.TrmMeshes[i];
                    if (!meshObj.Mesh) continue;

                    bool cancel = EditorUtility.DisplayCancelableProgressBar("Vertex Lightmapper", $"Calculating TRM Lighting ({meshObj.Transform.name} {i + 1} / {count})", (float) (i + 1) / count);
                    if (cancel)
                    {
                        Cleanup(true);
                        return;
                    ...
                    int[] tris = meshObj.Mesh.triangles;

                    Color[] colors = meshObj.Colors;

                    for (int j = 0; j < tris.Length - 3; j += 6)
                    {
                        Vector3 vert = Vector3.zero;

                        vert += meshObj.WorldVertices[tris[j + 0]];
                        vert += meshObj.WorldVertices[tris[j + 1]];
                        vert += meshObj.WorldVertices[tris[j + 2]];
                        vert += meshObj.WorldVertices[tris[j + 3]];
                        vert += meshObj.WorldVertices[tris[j + 4]];
                        vert += meshObj.WorldVertices[tris[j + 5]];
                        vert /= 6;

                        Vector3 normal = meshObj.WorldNormals[tris[j]];
                ...
                /*---Apply Colors---*/
                count = BakeData.SceneryMeshes.Count;
                for (int i = 0; i < count; ++i)
                {
                    VlmMeshObject meshObj = BakeData.SceneryMeshes[i];
                    if (!meshObj.Mesh) continue;
                    ...
                    /*---Setup Colors---*/
                    VlmData newData = BakeData.SceneryMeshes[i].Transform.gameObject.AddComponent<VlmData>();
                    newData.NewColors = meshObj.Colors;
                    newData.EncodeInTangents = meshObj.EncodeInTangents;
                    newData.UseVertexStream = meshObj.UseVertexStream;
                }
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    /*---Setup Colors---*/
                    VlmData newData = BakeData.TrmMeshes[i].Transform.gameObject.AddComponent<VlmData>();
                    newData.NewColors = meshObj.Colors;
                    newData.EncodeInTangents = meshObj.EncodeInTangents;
                    newData.UseVertexStream = meshObj.UseVertexStream;
                }
                ...
                /*---Save VCM Data---*/
                TrackDataStorage track = Object.FindObjectOfType<TrackDataStorage>();
                if (track) track.UpdateFloorColors();
        ...
        public static void Cleanup(bool removeIds)
        {
            ...
            foreach (VlmMeshObject mesh in BakeData.SceneryMeshes) mesh.Cleanup();
