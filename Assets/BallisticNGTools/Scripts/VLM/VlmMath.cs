...
namespace Vlm
{
    ...
    public class VlmMath
    {
        ...
        public static Color CalculateColorSponge(Vector3 vertex, Vector3 normal, AdvancedLightSponge sponge)
        {
            if (!sponge) return Color.white;

            if (sponge.Shape == AdvancedLightSponge.SpongeShape.Box) return CalculateColorSpongeCubic(vertex, normal, sponge.transform.position, sponge.transform.rotation, sponge.BoxBounds, sponge.Intensity, sponge.IgnoreReverseNormals);
        ...
        public static VlmMeshObject GetMeshObjectFromRaycastHit(RaycastHit hit)
        {
            ...
            return VlmBakeData.Current.SceneryMeshes.FirstOrDefault(m => m.MeshCollider == collider);
        }
