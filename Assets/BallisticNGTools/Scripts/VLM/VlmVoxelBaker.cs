    ...
    public struct VlmVoxelBaker
    {
                    ...
                    for (int z = 0; z < ResZ; ++z)
                    {
                        ...
                        if (colliders.Length > 0)
                        {
                            ...
                            foreach (Collider collider in colliders)
                            {
                                ...
                                VlmMeshObject mObj = VlmBakeData.Current.SceneryMeshes.FirstOrDefault(m => m.MeshCollider == collider);
                                if (!mObj.Mesh) mObj = VlmBakeData.Current.TrmMeshes.FirstOrDefault(m => m.MeshCollider == collider);
        ...
        private void CalculateGi()
        {
            ...
            foreach (PropogationCone cone in Cones)
            {
                ...
                foreach (VlmMeshObject mObj in data.SceneryMeshes)
                {
                ...
                foreach (VlmMeshObject mobj in data.TrmMeshes)
                {
                    for (int j = 0; j < mobj.Triangles.Length - 3; j += 6)
                    {
                        Vector3 vert = Vector3.zero;
                        trmTris[0] = mobj.Triangles[j + 0];
                        trmTris[1] = mobj.Triangles[j + 1];
                        trmTris[2] = mobj.Triangles[j + 2];
                        trmTris[3] = mobj.Triangles[j + 3];
                        trmTris[4] = mobj.Triangles[j + 4];
                        trmTris[5] = mobj.Triangles[j + 5];

                        vert += mobj.WorldVertices[trmTris[0]];
                        vert += mobj.WorldVertices[trmTris[1]];
                        vert += mobj.WorldVertices[trmTris[2]];
                        vert += mobj.WorldVertices[trmTris[3]];
                        vert += mobj.WorldVertices[trmTris[4]];
                        vert += mobj.WorldVertices[trmTris[5]];
                        vert /= 6;

                        float bounceIntensity = VlmMath.CalculateIntensitySpot(vert, cone.Pos, cone.Direction, VlmBakeData.BounceDistance, cone.ConeAngle);
                        Color bounceColor = new Color(bounceIntensity, bounceIntensity, bounceIntensity, 0.0f) * Source.color;

                        for (int i = 0; i < 6; ++i)
                        {
                            Color col = mobj.GiAdd[trmTris[i]];
                            if (col.r * col.g * col.b < bounceColor.r * bounceColor.g * bounceColor.b) mobj.GiAdd[trmTris[i]] = bounceColor;
                        }
                    }
                }
